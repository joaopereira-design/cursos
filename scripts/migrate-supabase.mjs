...
const catalogResult = await writeCatalogToSupabase(catalog);
console.log(`Catalogo enviado: ${catalogResult.courses} cursos, ${catalogResult.lessons} aulas/materiais.`);

if (Object.keys(progress || {}).length) {
  const progressResult = await writeWatchProgressToSupabase("local-owner", progress);
...
if (driveToken) {
  await saveSecretToSupabase("google_drive_token", driveToken);
